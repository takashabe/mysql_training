...
* 1つのMySQLサーバ(マスター)のデータを、1つまたは複数のMySQLサーバ(スレーブ)に複製出来る
...
1. マスター上にレプリケーション用ユーザを作成する(内容はマスターから作成する場合と同様)

2. 新しいスレーブで設定ファイルを作成する
...
4. 既存のスレーブから新しいスレーブへデータをコピーする

データディレクトリ( `/var/lib/mysql/` )を `scp` や `ftp` で新しいスレーブ上にコピーする

5. 既存のスレーブおよび新しいスレーブでmysqlを起動する

### ストレージのスナップショットをコピーする
...
2. マスター上にレプリケーション用ユーザを作成する

3. スナップショットを取得する

4. スナップショットボリュームからデータディレクトリをスレーブにコピーする

5. スレーブを起動する

6. スレーブ上でレプリケーションを設定する
...
# TODO: スレーブをマスターとして設定する方法、複数スレーブがある場合にどれをマスターにするかの判断について書く
