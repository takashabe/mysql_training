...
#### マスター上にレプリケーション用ユーザを作成する(内容はマスターから作成する場合と同様)
...
#### 既存のスレーブから新しいスレーブへデータをコピーする

データディレクトリ( `/var/lib/mysql/` )を `scp` や `ftp` で新しいスレーブ上にコピーする

#### 既存のスレーブおよび新しいスレーブでmysqlを起動する
...
#### マスター上にレプリケーション用ユーザを作成する

#### スナップショットを取得する

#### スナップショットボリュームからデータディレクトリをスレーブにコピーする
...
#### 最も進んでいるスレーブをマスターに昇格する

スレーブが1台の場合同様、スレーブをマスターにセットアップします

#### 他スレーブの設定を変更し、新マスターを見るようにする
